...
public class PcapReplayer implements AutoCloseable {
  ...
  public TigerProxy replayWithDirectForwardUsing(TigerProxyConfiguration tigerProxyConfiguration) {
    try (val serverSocket = buildServerSocket()) {
      ...
      int before = tigerProxy.getMessages().size();
      tigerProxy.waitForAllCurrentMessagesToBeParsed();
      int after = tigerProxy.getMessages().size();
  ...
  private Socket replayPackets(Socket clientSocket, ServerSocket serverSocket) {
    ...
    AtomicInteger bytesSendFromClientCount = new AtomicInteger();
    AtomicInteger bytesSendFromServerCount = new AtomicInteger();
    ...
    for (val packet : toBeReplayedPackets) {
      if (packet.isServerRequest) {
        ...
        readNumberOfBytesFromSocket(bytesSendFromServerCount.get(), clientSocket, "client")
            .thenAccept(result -> result.ifPresent(bytes -> { receivedPacketsInClient.add(bytes); bytesSendFromServerCount.addAndGet(-bytes.length); })).join();
        log.atInfo()
        ...
        bytesSendFromClientCount.addAndGet(packet.payload.length);
        ...
        readNumberOfBytesFromSocket(bytesSendFromClientCount.get(), serverConnectionSocket, "server")
            .thenAccept(result -> result.ifPresent(bytes -> { receivedPacketsInServer.add(bytes); bytesSendFromClientCount.addAndGet(-bytes.length); })).join();
        log.atInfo()
            .addArgument(() -> new String(packet.payload).lines().findFirst().get())
            .log("Sending packet to client.... ({})");
        serverConnectionSocket.getOutputStream().write(packet.payload);
        bytesSendFromServerCount.addAndGet(packet.payload.length);
        serverConnectionSocket.getOutputStream().flush();
      }
    }
    readNumberOfBytesFromSocket(bytesSendFromServerCount.get(), clientSocket, "client")
        .thenAccept(result -> result.ifPresent(receivedPacketsInClient::add)).join();
    readNumberOfBytesFromSocket(bytesSendFromClientCount.get(), serverConnectionSocket, "server")
        .thenAccept(result -> result.ifPresent(receivedPacketsInServer::add)).join();
  ...
  private CompletableFuture<Optional<byte[]>> readNumberOfBytesFromSocket(
    ...
    if (bytesCount > 0) {
      return CompletableFuture.supplyAsync(() -> readBytesWithTimeout(bytesCount, socket, 5000, message));
    } else {
  ...
  /**
   * Reads up to bytesCount bytes from the socket's InputStream within the given timeout.
   * If the timeout is reached before bytesCount bytes are read, returns the bytes read so far.
   */
  public static Optional<byte[]> readBytesWithTimeout(int bytesCount, Socket socket, long timeoutMillis, String message) {
    ...
    try {
      ...
      return bytesRead == 0 ? Optional.empty() : Optional.of(bytesRead == bytesCount ? buffer : Arrays.copyOf(buffer, bytesRead));
  }
